...
async function run() {
    try {
        ...
        app.put('/users/admin', verifyToken, async (req, res) => {
            ...
            if (requister) {
                const requisterAcount = await usersCollection.findOne({ email: requister })
                if (requisterAcount.role === 'admin') {
                ...
                res.status(401).json({ message: 'you do not have to access make admin' })
            }
        })
